 ...
 * Time: 8:00 AM
 */
public class WallAnalyzer extends ClassAnalyzer
{

    public WallAnalyzer(Analyzer instance)
    {
        super(instance);
    }
    ...
    {
        if (node.name.equals(getInstance().getAccessors().get("WallAccessor").name))
        {
            return new ClassIdentity("Wall", node.name, node, 0);
        }
        return null;
    }
    ...
    {
        return new String[]{"WallAccessor", "RenderableAccessor"};
    }
}
